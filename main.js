//// USTAWIENIE TŁA - POCZĄTEK ////

// Some random colors
const colors = ["#ff0560", "#00ff00", "#0000ff","#f00080", "#f0f0f0", "#00000f",];
...
function losujKolor(){ //Funkcja służąca do wylosowania trzech randomowych wartości RGB i
                        //ustawienia tego koloru w boxie
  r = Math.floor(Math.random()*255);
  g = Math.floor(Math.random()*255);
  b = Math.floor(Math.random()*255);
  var kolor = "rgb("+r+","+g+","+b+")";
  $("#box").css("background",kolor); // ustawienie wylosowanego koloru w boxie
  //$(".wylosowaneWartosci").text(kolor);

  $("#zatwierdz").css({"opacity":"100%", "cursor":"pointer"}); // zablokowanie przycisku "Zatwierdz"
...
function pobierzOdpowiedzi(){ //Funkcja służąca do pobrania wartości RGB wpisanych przez użytkownika
                              // i sprawdzenia czy są w odpowiednich zakresach
  result = true;
...
$(document).ready(function(){
  ...
  $("#zatwierdz").click(function(){
    ...
    if (liczbaProb===0){
      ...
      $("#wynik").text("Najlepszy wynik: "+Math.max.apply(Math,tablicaWynikow));
